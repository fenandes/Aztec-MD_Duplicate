const { proto, getContentType } = require('@whiskeysockets/baileys').WAMessageProto;
...
function serialize(msg, vorterx) {
  if (msg.key) {
    ...
    msg.from = proto.jidDecode(msg.key.remoteJid);
    msg.isGroup = msg.from.endsWith('@g.us');
    msg.sender = msg.isGroup ? proto.jidDecode(msg.key.participant) : msg.isSelf ? proto.jidDecode(vorterx.user.id) : msg.from;
  }
  if (msg.message) {
    ...
    try {
      ...
      if (quoted.quotedMessage['ephemeralMessage']) {
        ...
        if (tipe === 'viewOnceMessageV2') {
          msg.quoted = {
            ...
            participant: proto.jidDecode(quoted.participant),
      ...
      msg.quoted.isSelf = msg.quoted.participant === proto.jidDecode(vorterx.user.id);
