const { jidDecode } = require('@whiskeysockets/baileys').proto;
const { getContentType } = require('@whiskeysockets/baileys').WAMessageProto;
...
function serialize(msg, vorterx) {
  if (msg.key) {
    ...
    msg.from = jidDecode(msg.key.remoteJid);
    msg.isGroup = msg.from.endsWith('@g.us');
    msg.sender = msg.isGroup ? jidDecode(msg.key.participant) : msg.isSelf ? jidDecode(vorterx.user.id) : msg.from;
  }
  if (msg.message) {
    ...
    try {
      ...
      if (quoted.quotedMessage['ephemeralMessage']) {
        ...
        if (tipe === 'viewOnceMessageV2') {
          msg.quoted = {
            ...
            participant: jidDecode(quoted.participant),
      ...
      msg.quoted.isSelf = msg.quoted.participant === jidDecode(vorterx.user.id);
    ...
    msg.body =
     ...
     '';
    msg.text = msg.body;
    if (msg.type === 'buttonsResponseMessage') {
      msg.buttons = msg.message[msg.type]?.selectedDisplayText;
    }}
   return msg;
}
