...
module.exports = MessageHandler = async (messages, vorterx) => {
 ...
 function fetchRandomBotConfiguration() {
  const botIDs = Object.keys(vorterx);
  const randomBotID = botIDs[Math.floor(Math.random() * botIDs.length)];
  const randomBotConfiguration = vorterx[randomBotID];

  return randomBotConfiguration;
}

const randomBotConfiguration = fetchRandomBotConfiguration();

console.log("Bot Name:", randomBotConfiguration.botName);
console.log("Author:", randomBotConfiguration.author);
    //=====//////====    
      if(body == prefix) {
      await xReact("💢");
