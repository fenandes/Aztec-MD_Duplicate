...
const { getRandom, fetchBuffer } = require("../../mangoes/myFunc.js")

class WAConnection {constructor(vorterx) {
for (let v in vorterx) {this[v] = vorterx[v]
}}async serializeM(m) {return exports.serialize(this, m)
}
parseMention(text) {
return [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net')
 ...
 if (fileName) {let ftype = await FileType.fromBuffer(buffer)
...
exports.serialize = (vorterx, m, options = {}) => {
if (!m) return m
let M = proto.WebMessageInfo
m = M.fromObject(m)if (m.key) {
m.from = jidNormalizedUser(m.key.remoteJid || m.key.participant)
m.fromMe = m.key.fromMe
m.id = m.key.id
m.isBot = m.id.startsWith("BAE5") && m.id.length == 16
m.isGroup = m.from.endsWith("@g.us")
m.sender = jidNormalizedUser(m.fromMe && vorterx.user?.id || m.key.participant || m.from || "")}
...
if (m.quoted) {m.quoted.type = getContentType(m.quoted)
...
m.quoted.fromMe = (m.quoted.sender == jidNormalizedUser(vorterx.user && vorterx.user?.id))
...
key: {remoteJid: m.quoted.from,fromMe: m.quoted.fromMe,id: m.quoted.id
},message: m.quoted,...(m.quoted.isGroup ? { participant: m.quoted.sender } : {})})
m.quoted.delete = () => vorterx.sendMessage(m.quoted.from, { delete: vM.key })
m.quoted.download = (pathFile) => vorterx.downloadMediaMessage(m.quoted.msg, pathFile)}}
m.download = (pathFile) => vorterx.downloadMediaMessage(m.msg, pathFile)
...
m.reply = (text, chatId = m.from, options = {}) => Buffer.isBuffer(text) ? vorterx.sendFile(chatId, text, 'file', '', m, { ...options }) : vorterx.sendText(chatId, text, m, { ...options })
return m
}
