...
module.exports = MessageHandler = async (messages, vorterx) => {
  try {
    ...
    const mentionByTag = () => {return messages.type === 'extendedTextMessage' && messages.messages[0].message.extendedTextMessage.contextInfo != null? messages.messages[0].message.extendedTextMessage.contextInfo.mentionedJid: [];};
  ...
  mime = "";
   ...
   try {
    ...
    command.xstart(vorterx, {
      name: "vorterx",
      pushName: m.pushName,
      arg,text,participants,mentionByTag,mime,quoted,isAdmin,
      isBotAdmin,xReact
    });
  }
} catch (err) {
